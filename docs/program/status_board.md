...
| WLD | World Systems and Campaign Platform | in_progress | 5H-world-systems | `WLD-03` is `pr_open`; `WLD-01`, `WLD-02`, and `WLD-09` are `in_progress`; remaining WLD tasks are `not_started` in `backlog.csv`. |
...
| WLD-09 | codex/feat/wld-09-build-multi-encounter-adventuring-day-persistence | world_state_b | in_progress | Implementing campaign runtime persistence and deterministic short/long-rest recovery flow across encounter checkpoints. |
...
- `WLD-09` is active and depends on `WLD-03`, `DBS-04`, and `DBS-05` per `backlog.csv`.
