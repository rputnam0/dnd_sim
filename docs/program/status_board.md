...
| CAP | Capability Manifest | in_progress | 5C-capability-manifest | `CAP-01` and `CAP-03` are `pr_open`, while `CAP-04` is `in_progress` in `backlog.csv`; remaining CAP tasks are `not_started`. |
