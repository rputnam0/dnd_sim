...
| ARC | Runtime Decomposition | in_progress | 5B-runtime-decomposition | `ARC-01` and `ARC-02` are `in_progress`; remaining ARC tasks are `not_started` in `backlog.csv`. |
...
- No active DOC blockers remain; ARC-01 and ARC-02 runtime decomposition work is currently in progress on assigned branches.
