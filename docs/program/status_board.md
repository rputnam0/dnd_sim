...
| OBS | Replay, Logging, and Observability | in_progress | 5D-observability | `OBS-02`, `OBS-03`, `OBS-04`, and `OBS-05` are `pr_open`; `OBS-06` is `in_progress`; remaining OBS tasks are `not_started` in `backlog.csv`. |
...
| OBS-06 | `codex/feat/obs-06-build-replay-bundle-writer-loader-and-diff-harness` | observability_replay | in_progress | Implementing replay bundle schema/writer/loader and deterministic diff harness under OBS-06. |
...
- No active DOC blockers remain; OBS-02 through OBS-05 are in PR review and OBS-06 is active.
...
- `OBS-06` is active and depends on `OBS-02`, `OBS-03`, `OBS-05`, and `ARC-08` per `backlog.csv`.
