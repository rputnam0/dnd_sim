...
| FIN | Completion Gates | pr_open | 5I-completion-gates | `FIN-01` through `FIN-05` are `merged`; `FIN-06` is `pr_open` for release baseline cutover and completion closeout. |

## Active branches

| Task ID | Branch | Owner | Status | Notes |
|---|---|---|---|---|
| FIN-06 | codex/feat/fin-06-cut-release-baseline-archive-prior-program-artifac | release_lead | pr_open | FIN-06 release baseline cutover, archive updates, and final completion-gate synchronization are in PR review. |

## Open PRs

| Task ID | PR | Owner | Gate status | Notes |
|---|---|---|---|---|
| FIN-06 | [#TBD](https://github.com/rputnam0/dnd_sim/pulls) | release_lead | pending | FIN-06 final release-baseline cutover and completion-closeout updates are in PR review. |
...
- `FIN-01`, `FIN-02`, `FIN-03`, `FIN-04`, and `FIN-05` are merged in `backlog.csv`.
- `FIN-06` is in PR review after dependency clearance and full completion-gate synchronization.
