...
| OBS | Replay, Logging, and Observability | pr_open | 5D-observability | `OBS-02` through `OBS-07` are `pr_open` in `backlog.csv`. |
...
| OBS-07 | `codex/feat/obs-07-establish-golden-trace-corpus-and-trace-review-gat` | observability_gate | pr_open | Golden trace corpus coverage and replay trace drift review gate tooling are implemented and under review. |
...
| OBS-04 | [#131](https://github.com/rputnam0/dnd_sim/pull/131) | observability_ai | pending | Emits AI candidate scoring and rationale trace envelopes with full candidate-set rows and selected/rejected rationale metadata. |
| OBS-05 | [#130](https://github.com/rputnam0/dnd_sim/pull/130) | observability_d | pending | Emits structured `resource_delta`, `rng_audit`, and `invariant_violation` telemetry with deterministic tests. |
| OBS-06 | [#137](https://github.com/rputnam0/dnd_sim/pull/137) | observability_replay | pending | Adds replay bundle schema/writer/loader and deterministic replay diff harness for run-to-run bundle comparisons. |
| OBS-07 | [#139](https://github.com/rputnam0/dnd_sim/pull/139) | observability_gate | pending | Adds golden trace corpus artifacts plus deterministic verification gate with explicit drift-approval workflow. |
...
- No active DOC blockers remain; OBS-02 through OBS-07 are in PR review.
...
- `OBS-07` is in PR review and depends on `OBS-06` and `DOC-05` per `backlog.csv`.
