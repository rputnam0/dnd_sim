...
def test_chr13_integration_quickened_sequence_is_legal_and_deterministic(
    ...
    resources = trial.state_snapshots[-1]["party"]["sorcerer_5"]["resources"]

    assert trial.resources_spent["sorcerer_5"].get("sorcery_points", 0) == 2
    assert trial.resources_spent["sorcerer_5"].get("spell_slot_1", 0) == 1
