...
def _w6_par_05h1_trait_ids() -> set[str]:
    with REGISTRY_PATH.open(newline="", encoding="utf-8") as handle:
        ids = {
            str(row["content_id"])
            for row in csv.DictReader(handle)
            if str(row.get("leaf_task_id", "")).strip() == "W6-PAR-05H1"
        }
    assert len(ids) == 19
    return ids


def test_w6_par_05h1_trait_records_are_non_executable_metadata() -> None:
    owned_ids = _w6_par_05h1_trait_ids()
...
def test_w6_par_05h1_trait_files_use_meta_rows_only() -> None:
    owned_ids = _w6_par_05h1_trait_ids()
