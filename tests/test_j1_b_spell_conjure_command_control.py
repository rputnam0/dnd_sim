...
EXPECTED_EFFECT_TYPES = {
    "conjure_woodland_beings": {"conjure"},
    "control_flames": {"apply_condition"},
    "control_weather": {"apply_condition"},
    "control_winds": {"hazard"},
    "create_homunculus": {"summon"},
    "create_magen": {"summon"},
    "create_undead": {"summon", "command_allied"},
    "dominate_beast": {"apply_condition"},
    "dominate_monster": {"apply_condition"},
    "dominate_person": {"apply_condition"},
}
...
def test_j1_b_batch_records_are_supported() -> None:
...
def test_j1_b_batch_uses_canonical_rows() -> None:
    ...
    for slug in ("dominate_beast", "dominate_monster", "dominate_person"):
        ...
        assert effects, f"{slug} should apply a domination condition"
        assert any(
            str(effect.get("apply_on", "")).strip().lower() == "save_fail" for effect in effects
        ), f"{slug} should be save-gated"


def test_j1_b_conjure_woodland_beings_runtime_creates_concentration_linked_ally() -> None:
    ...
    action = _action_from_payload(_spell_payload("conjure_woodland_beings"))
...
def test_j1_b_create_undead_runtime_requires_command() -> None:
    caster = _actor("caster", "party")
    enemy = _actor("enemy", "enemy")
    actors = {caster.actor_id: caster, enemy.actor_id: enemy}
    damage_dealt, damage_taken, threat_scores, resources_spent = _trackers(caster, enemy)

    action = _action_from_payload(_spell_payload("create_undead"))
    ...
    assert summoned.requires_command is True
